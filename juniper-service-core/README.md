...
| WebSocket | `attach_websocket`, `training_stream_handler`, `control_stream_handler`, … | Live training + control streams + `/ws/workers`. Control WS `ws_control_rate_limit_per_sec` → `LeakyBucket`; open [ml#977](https://github.com/pcalnon/juniper-ml/pull/977) fixes `refill_rate=0` (`retry_after=3600`). See [REFERENCE § Control WS LeakyBucket](../docs/REFERENCE.md#control-ws-leakybucket-ws_control_rate_limit_per_sec). |
