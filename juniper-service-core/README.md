...
| WebSocket | `attach_websocket`, `training_stream_handler`, `control_stream_handler`, … | Live training + control streams, plus a worker channel (`/ws/workers`). Control WS rate limit: `ws_control_rate_limit_per_sec` → `LeakyBucket`; open [juniper-ml#977](https://github.com/pcalnon/juniper-ml/pull/977) makes `refill_rate=0` return `retry_after=3600` (pre-fix ZeroDivisionError). Operator contract: [`docs/REFERENCE.md` § Control WS LeakyBucket](../docs/REFERENCE.md#control-ws-leakybucket-ws_control_rate_limit_per_sec). |
