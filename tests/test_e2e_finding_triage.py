...
# `withdrawn` is matched but not returned. It arrived after this suite was written and the
# regex, requiring `accepted` to be followed directly by `open`, then matched NOTHING -- so
# every assertion failed on "totals block missing" rather than on its own claim. Requiring
# the line (not making it optional) means a disposition vanishing from the operator summary
# fails here, which is what this suite is for.
_TOTAL = re.compile(
    r"total findings : (\d+)\n" r"  fixed        : (\d+)\n" r"  accepted     : (\d+)\n" r"  withdrawn    : \d+\n" r"  open         : (\d+)",
)
...
class StatusTokensAndIdentity(unittest.TestCase):
    ...
    def test_first_heading_wins_so_a_later_fixed_cannot_close_an_open(self) -> None:
        note = "**F-CANOPY-010 — still broken (P1, OPEN).**\n" "**F-CANOPY-010 — later closed (P1, FIXED).**\n"
        result = _run(note)
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_letter_suffix_ids_are_distinct(self) -> None:
        note = "**F-CANOPY-027 — original mount-order defect (P1, FIXED).**\n" "**F-CANOPY-027a — follow-on starvation (P1, OPEN).**\n"
        result = _run(note)
        self.assertEqual(result.returncode, 0, result.stderr)
