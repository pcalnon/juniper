...
_SPREAD = (
    "Do not set output_epochs_alias to be thorough. "
    "Filler sentence one about ordinary configuration and nothing else here. "
    "Filler sentence two about ordinary configuration and nothing else here. "
    "That choice silently diverges the service from the CLI path. "
    "Filler sentence three about ordinary configuration and nothing else here. "
    "This destroys the live importer and cannot be undone."
)
...
class ScoreWindowTest(unittest.TestCase):
    def test_block_score_beats_every_two_sentence_window(self):
        ...
        self.assertLess(
            max(window_scores),
            block_n,
            f"a 2-sentence window already equals the block ({max(window_scores)} vs {block_n}); "
            "fixture is not the documented miss",
        )
    ...
    def test_demotion_never_drops_a_remaining_score(self):
        ...
        win = (
            "Do not log this unique_hazard_token. "
            "logger.warning at WARNING level goes to the log shipper for operators."
        )
        n, hits, demoted = mod.score_window(win)
        self.assertEqual(n, 1)
