            ...
            json.dumps(
                {
                    ...
                    "outcome": "succeeded",
                    # `main` gained a termination-branch precondition after this suite was
                    # written: step_count is deterministic only WITHIN a branch, so cells
                    # with no `completion_reason` all land on the `None` branch and
                    # `build_baseline` refuses the suite before any test here reaches its
                    # own assertion. Annotating them keeps the refusal these tests DO want
                    # (recurrence is uncountable) as the one that fires.
                    "completion_reason": "below_threshold",
                    "timings": {"drive": 65.0},
                    "drive_loop": {"polls": 14},
