...
_FAILING_LINE = "BET-FAILING  seeded=43/50 rate=60.5% ci=[0.456, 0.736] " "escalations=0  (upper bound below the boundary)"
_HOLDS_LINE = "HOLDS-AT-0.75  seeded=50/50 rate=82.0% ci=[0.760, 0.880] " "escalations=0  (lower bound at or above the boundary)"
_PROGRESS_LINE = "IN-PROGRESS  seeded=10/50 rate=n/a ci=n/a escalations=0  " "(10/50 runs; a later BET-FAILING mention must not trip the guard)"
...
class StoppingRuleCli(unittest.TestCase):
    ...
    def test_bet_failing_refuses_and_does_not_dispatch(self) -> None:
        # No --dry-run: a real invocation is what the rule rations, and it is the
    ...
    def test_dry_run_is_EXEMPT_from_the_guard(self) -> None:
        """INVERTED on harvest, and the inversion is the point.

        This asserted that --dry-run still refuses -- a characterization of the
        ordering as it stood. Main reversed it deliberately: the rule rations
        billed sessions and a dry run spends none, so gating it was refusing
        something that costs nothing. The reversal is not free to undo, either;
        `refuses_terminal_verdict`'s comment records that gating --dry-run made it
        "exit 2 with EMPTY STDOUT" and broke the two DryRunDoesNotLeakTheTask
        tests on every Python in CI.

        So: pin the exemption. A reorder that re-gates --dry-run breaks here,
        which is the same protection the original wanted, pointed the right way.
        """
        rc, err, n = _run_main(["--dry-run"], _FAILING_LINE)
        self.assertEqual(rc, 0)
        self.assertNotIn("REFUSING", err)
        self.assertGreaterEqual(n, 1)
        # The pure predicate still says the verdict IS terminal -- the exemption is
        # about this invocation, not about the verdict.
        self.assertEqual(mod.terminal_verdict(_FAILING_LINE), "BET-FAILING")
