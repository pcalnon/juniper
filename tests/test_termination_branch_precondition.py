...
def _write_run(root: Path, run_id: str, *, step_count=1770, step_sum=63.0, reason="early_stopped", omit_reason=False, outcome="succeeded") -> Path:
    """One finished cell.

    ``outcome`` and ``reason`` are DIFFERENT fields and hold different vocabularies.
    ``outcome`` is the driver's verdict (``succeeded`` / ``timed_out`` / ``torn_down_early``
    / ``stalled``); ``completion_reason`` comes from the cascor service's training status
    (``early_stopped`` / ``below_threshold`` / ...), set at run_experiment.py:1716. The
    truncating states are driver-side, so they belong in ``outcome``.
    """
    run_dir = root / run_id
...
class ReaderExtractTest(unittest.TestCase):
    ...
    def test_missing_null_and_empty_reasons_are_their_own_branch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            # NOT []. Filtering unknown out before taking uniqueness is the fail-open this
            # guard exists to prevent -- it makes `4x early_stopped + 1x None` read as one
            # branch. Unknown is its own value and is reported as one.
            self.assertEqual(summary["completion_reasons"], ["", "None"])
...
class SummariseBranchTest(unittest.TestCase):
    ...
    def test_truncated_set_is_only_the_driver_stops(self):
        ...
        # Driven through `outcome`. A `completion_reason` of "timed_out" is a shape
        # production never writes, and matching on it made this guard unable to fire.
        rows = [
            {"outcome": "succeeded", "completion_reason": "early_stopped", "step_count": 100, "work_countable": True},
            {"outcome": "timed_out", "completion_reason": None, "step_count": 100, "work_countable": True},
            {"outcome": "succeeded", "completion_reason": "below_threshold", "step_count": 100, "work_countable": True},
        ]
    ...
    def test_all_same_truncating_reason_is_still_truncated(self):
        ...
        rows = [{"outcome": "timed_out", "completion_reason": "max_iterations", "step_count": 50, "work_countable": True} for _ in range(2)]
        summary = rrm.summarise(rows)
        self.assertTrue(summary["single_completion_reason"])
    ...
    def test_a_partially_annotated_suite_is_not_a_single_branch(self):
        ...
        summary = rrm.summarise(rows)
        # The unannotated neighbour appears as "None" rather than vanishing: an operator
        # reading "different branches ['None', 'early_stopped']" can see that one cell was
        # never annotated, which a bare ["early_stopped"] would hide.
...
class MakeBaselineBranchTest(unittest.TestCase):
    ...
    def test_refuses_each_truncating_outcome(self):
        """A driver-truncated run measures the BUDGET, so a baseline must not enshrine it.

        Driven through ``outcome``, not ``completion_reason``. The truncating names are the
        driver's own states and that is the field it writes them to; a manifest carrying
        ``completion_reason: "timed_out"`` is a shape nothing produces.
        """
        for reason in sorted(rrm.TRUNCATING_TERMINATIONS):
            with self.subTest(reason=reason), tempfile.TemporaryDirectory() as tmp:
                suite = _write_suite(Path(tmp), "s", [{"outcome": reason}, {"outcome": reason}])
