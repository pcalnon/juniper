...
_ASSIGN = re.compile(r"\b(?P<name>JUNIPER_DATA_EQUITIES_(?:MAX_SYMBOLS|ALLOW_TRUNCATION|CACHE_DIR))\s*=")
...
class EquitiesVerdictUnitTest(unittest.TestCase):
    ...
    def test_allow_truncation_opts_in(self) -> None:
        ...
        fifteen = [f"S{i:02d}" for i in range(SYMBOL_CAP + 1)]
        self.assertEqual(
            equities_cell_verdict({"dataset.generator": "equities", "dataset.params": {"symbols": fifteen, "allow_truncation": True}}),
            "ok-opt-in",
        )
