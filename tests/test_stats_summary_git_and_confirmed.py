...
RENAMED ON HARVEST. The cursor fleet authored this suite and
``tests/test_stats_summary_render.py`` about an hour apart, both under the same
filename, as two INDEPENDENT takes on the same renderer. They share not one test
name, and neither dominates: this file uniquely covers the ``confirmed`` tri-state
(``None`` is not ``False``; a true value does not fall back to N/A), git
provenance when git is unavailable, the clean-head case that must omit the dirty
flag, and a DECREASING count that must not mint a poll sample. Its sibling
uniquely covers the conditional class-distribution line, the reason sub-bullet,
the ``present``/``written`` key precedence, and a rename of ``written`` onto
``confirmed``. Both were kept rather than picking one and discarding real coverage.

``StepDurationEdgeTest`` exists in both files. That is legal -- the classes are
module-qualified, and juniper-ml invokes each suite as its own
``python3 -m unittest -v tests/<file>`` -- but do not merge these files without
reconciling that name.
