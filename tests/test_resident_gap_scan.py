...
_GAP = (
    "CRITICAL: never silently drop alpha_hazard_token when the finder walks "
    "this file and the comment is long enough to clear min-len"
)
_GAP_COMMENT = "# " + _GAP + "\n"

# Same shape, more distinctive identifiers -- ranks first under identifier-count sort.
_GAP_RICH = (
    "CRITICAL: never silently drop bravo_hazard_token or CHARLIE_HAZARD_TOKEN "
    "or delta_hazard_token when the finder ranks by identifier count"
)
...
class IdentifiersTest(unittest.TestCase):
    def test_prose_backticks_are_not_identifiers(self):
        ...
        text = (
            "CRITICAL: do not accept `), and the verbatim rejection detail (` "
            "as a name, it silently inflates every candidate"
        )
    ...
    def test_snake_and_constant_are_harvested(self):
        text = (
            "CRITICAL: never silently alias OUTPUT_EPOCHS_SPLIT onto "
            "output_epochs_alias in this path"
        )
        found = mod.identifiers(text)
    ...
    def test_stop_drops_no_update(self):
        ...
        text = (
            "CRITICAL: never silently treat no_update as an identifier in this "
            "comment block about the Dash execution model"
        )
        found = mod.identifiers(text)
