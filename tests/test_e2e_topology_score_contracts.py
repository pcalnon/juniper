...
class DriverWiresTheContractsTest(unittest.TestCase):
    def test_driver_calls_the_three_scorers(self) -> None:
        ...
        called = {node.attr for node in ast.walk(tree) if isinstance(node, ast.Attribute) and node.attr in {"score_m_topology_03", "score_m_topology_14", "score_m_topology_18"}}
        self.assertEqual(
            called,
