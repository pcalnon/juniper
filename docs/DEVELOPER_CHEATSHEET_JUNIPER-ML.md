...
| `python util/experiments/make_baseline.py --tag TAG --suite SUITE_DIR --dry-run` | Validate a Q-8 baseline; **refuses** a recurrence suite (no countable work) |
| `python util/experiments/compare_baseline.py --baseline TAG --suite SUITE_DIR` | Split compare; recurrence candidate → REFUSED exit 2 (speed alone is not gated) |
...
Tip: recurrence work is **not countable** (juniper-ml#1683). `n_epochs` is 1-or-200 by readout type; `n_windows` is input size. `read_run_metrics.py --run RUN_DIR --json` reports `work_countable: false`; `make_baseline` / `compare_baseline` refuse rather than gate PF-5/6/7 on speed. Do not cut a speed-only baseline. See [REFERENCE — Recurrence Work Is Not Countable](REFERENCE.md#recurrence-work-is-not-countable).
