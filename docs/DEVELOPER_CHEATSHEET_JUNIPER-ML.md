...
**Sequence-safety / fleet triage (juniper-ml#895 / #908):** `predict_merge` shells out to
`util/sequence_safety/symbol_loss_check.py` on the merged RESULT (byte-identical to post-merge
`main-verify`). Checker missing / exit `2` / empty / non-JSON → `ast_symbol_screen.status=skip`
(**not** `DAMAGED-FIX-FIRST`). Intentional symbol removals need an `Allow-Symbol-Loss: <qualified.symbol>`
commit trailer — the per-PR `allow-symbol-loss` label is WARN-only advisory and does **not** green
`main-verify`. Skip local pre-commit with `JUNIPER_FLEET_SKIP_PRECOMMIT=1`. Full contract:
...
| `ast_symbol_screen.status=skip` looks like damage | It is **not** — only `status=fail` drives `DAMAGED-FIX-FIRST` (#908). Fix missing/broken `symbol_loss_check.py`. |
