...
- [Experiment Stack Utilities](REFERENCE.md#experiment-stack-utilities) -- per-run cascor/recurrence experiments via `util/experiment_stack.bash` + `run_experiment.py` (plot `ValueError` → SKIP: [Plot SKIP contract](REFERENCE.md#plot-skip-vs-acceptance-valueerror-contract))
