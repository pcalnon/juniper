...
- `tests/test_soak_run_probe_scorer_redaction.py` -- Hermetic gate for the scoring-packet redaction in `util/soak_run_probe.py`. `--reveal` prints `post-interv. : N run(s)`; an earlier version embedded that stdout verbatim so the isolated scorer read a coverage tally beside the discriminator (finding 3 of `notes/JUNIPER_2026-09-02_JUNIPER-ML_SOAK-SESSION-ROLE-AUTOMATION-ANALYSIS.md`). Pins `startswith("post-interv.")` (a whole-line `in` search would drop a discriminator that names the coverage line) and that the live `--reveal` prefix still matches. Complementary to `tests/test_soak_run_probe.py`, which never assembles a scoring packet.
