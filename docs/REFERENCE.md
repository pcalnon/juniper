...
- `tests/test_e2e_unfilled_rows.py` -- Hermetic gate for `util/ad-hoc/e2e_unfilled_rows.py`, the AUTHORITATIVE ledger reader for remaining canopy E2E matrix rows (`util/` is outside every pre-commit Python hook). Pins fill's PLACEHOLDERS as UNFILLED and PASS/FAIL/INCONCLUSIVE as filled, that matrix-cell `pending` is already-written (same overwrite gate as fill), that a TSV `pending ...` record cannot credit a placeholder row (the Segment 15 estimator incident), status-by-header-name, escaped `\|`, `C2.`/`M-` namespaces only, prefix near-miss (`M-TOPOLOGY-01` vs `-010`), and per-`###` grouping that does not inherit the previous table's status index.
