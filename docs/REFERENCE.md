...
- `tests/test_stats_summary_render.py` -- Complementary leftover `StatsSummaryUnitTest` / `MetricsScrapedTest` (in `tests/test_run_experiment.py`) cannot see. Those suites pin percentiles / delta stats / the `_metrics_scraped` producer and call `render_summary_md` once for run-id / `## recurrence` / `n_windows` only. This suite pins the `#1550` honesty line an operator actually reads (`target_file_written` must not stand in for `scrape_confirmed`; `None` is not `False`; a missing key is `n/a (pre-2026-09-01 manifest)`), `last_count==0` / decreasing-count step stats, git dirty vs unavailable, and cascor/recurrence render edges (empty per-round, completion reason, disabled vs present crossval, explicit theta). No production edits.
...
│   ├── test_stats_summary_render.py        # Complementary leftover StatsSummaryUnitTest / MetricsScrapedTest cannot see: render_summary_md scrape tri-state (#1550), last_count==0 / decreasing count, git dirty/unavailable, cascor/recurrence render edges; hermetic
