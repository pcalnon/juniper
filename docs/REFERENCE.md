...
- `tests/test_soak_wilson_resolving.py` -- Hermetic gate for the Wilson *power* contract on `util/soak_ledger.py` that `test_soak_ledger.py` cannot see (interval-at-observed-rate vs power over the sampling distribution). Pins the low-side resolving threshold `k ≤ 1` at n=8, 9 **and** 10 (loosens to `k ≤ 2` only at n=11); power at p=0.25 falling 0.3671 → 0.3003 → 0.2440 then jumping at n=11; 26/42's 2.7e-06 clearance of the 0.75 boundary (a 4dp print cannot decide its side); 27/49 still BET-FAILING; and `binom_sf` `P(X >= k)` edges. `util/` is outside every pre-commit Python hook, so this suite is the gate.
