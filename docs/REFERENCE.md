...
- `tests/test_e2e_finding_triage.py` -- Hermetic gate for `util/ad-hoc/e2e_finding_triage.py`, the mechanical P0/P1 open-count for the canopy E2E Phase 2 exit criterion (`util/` is outside every pre-commit Python hook). Pins that ACCEPTED is a third disposition (not FIXED, not OPEN), `--open-only` hides FIXED and ACCEPTED but still prints full totals, HEALED counts as FIXED, and the first heading for an id wins.
  Also pins that `P0/P1` is not classified as `P0`, letter-suffix ids are distinct, untagged headers report `?`, and a `FIXED` token more than 170 characters before the end of the header body does not close an OPEN tail.
