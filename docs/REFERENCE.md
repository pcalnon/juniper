...
- `tests/test_e2e_matrix_fill.py` -- Hermetic gate for the canopy E2E matrix bulk-fill writer `util/ad-hoc/e2e_matrix_fill.py` (`util/` is outside every pre-commit Python hook).
  Escaped-pipe split must keep `display:block\|none` inside its cell so a fill lands in Status not FA (the C2.2-04 incident); status is located by header name per table (a hardcoded M-* index skips every C2.4 row).
  Dry-run writes nothing; without `--overwrite` a hand-authored cell is left alone; first `--verdicts` source wins; `pending`/`todo` prefixes are not verdicts; prefix near-miss (`M-TOPOLOGY-01` vs `-010`) is exact-id; compressed range/slash/lane-arm tokens expand; a leading-token bullet does not unpack a bare `-03` continuation.
  Distinct from `2026-09-02_matrix_set_verdicts.py` (single-row `--from`/`--set`) and `e2e_matrix_rescore.py` (named-row re-score). Never opens the live matrix.
...
- `util/ad-hoc/e2e_matrix_fill.py` -- Bulk-fills the `status` column of the canopy E2E click-by-click matrix from a run's `statuses.tsv` / rowlog. Status is located **by header name per table** (C2.4 vs M-* have different column sets); `\\|` stays inside its cell. Default is dry-run; without `--overwrite` an already-filled cell is left alone; `--verdicts` is repeatable and first source wins; `pending`/`todo` prefixes never reach a status cell. Tests: `tests/test_e2e_matrix_fill.py`.
