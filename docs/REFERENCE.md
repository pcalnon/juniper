...
`--batch` also builds a same-file cluster map and a restore/heal/repair/fix-first, least-colliding merge order (`_is_heal` tokens). Exit `0` always reports (even when every verdict is `DAMAGED` / `CONFLICT`); exit `2` is usage / precondition only (`gh` missing, bad `--repo-root`, unresolved ref).
...
Honesty pin: predict_merge's docs arm is still an **inline** removed-line counter on changed `.md` files — it does **not** shell out to `docs_additions_check.py` (heading / `--min-run` magnitude differs).
...
|---------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0.6.1   | 2026-08-05 | Fleet triage + sequence-safety operator surface; `_ast_symbol_screen` fail-soft / `skip`≠`DAMAGED` contract (#895 / #908); corrected verdict table vs `simulate_merge` |
