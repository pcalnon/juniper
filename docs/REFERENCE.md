...
- `tests/test_e2e_finding_triage_nested_bold.py` -- Hermetic leftover of the finding-triage dispositions and `pri_of` first-token suites: those headers never contain an inner `**CLOSED date**` (the F-CANOPY-037 close). The header regex is non-greedy to the first `**`, so nested bold truncates the body — CLOSED never reaches the parser (and is not a disposition), and the Phase 2 open-P0/P1 count then depends on whether `mechanism FIXED` appears before the inner `**`. Temp ledgers only; the live evidence note is never read.
