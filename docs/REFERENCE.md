...
- `tests/test_list_runs_classify_guards.py` -- Guards in the run classifier that the happy-path suite cannot reach: an unreadable manifest, a run with no terminal state, and the arms that decide a run is unclassifiable rather than defaulting it into a bucket.
- `tests/test_register_close_protocol.py` -- The defect-register close protocol operator pair: closing an entry requires BOTH halves shipped, and a one-sided close is refused rather than recorded.
...
- `tests/test_run_suite_gate_metrics.py` -- The suite-report gate VALUES, not just their presence: a report that prints a gate line computed from the wrong field reads as a pass to every human reader.
