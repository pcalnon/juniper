...
- `tests/test_work_countable_contract.py` -- Complementary pins for the work_countable third state (`util/experiments/{read_run_metrics,compare_baseline}.py`, P2 item 3.1 / ml#1683).
  The trigger PR already tests the reader row and `make_baseline` refusal; this suite pins the leftover those cannot see:
  `compare()` REFUSED (exit 2) with the honest "no countable work" reason rather than FAIL or "not a set of repeats",
  that a waiver cannot override it, that planted matching cascor histogram counts do not make an uncountable suite `work_invariant`,
  and that `drive` wins when both timing keys are present (misclassification would refuse a valid cascor gate).
  `util/` is not pre-commit-lint-gated, so this unittest is the gate.
...
│   ├── test_work_countable_contract.py     # Complementary: work_countable third state (compare REFUSED exit 2 / planted-count invariant / drive-wins detection; leftover #1683 cannot see)
