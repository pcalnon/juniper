...
- `tests/test_soak_analyse_date_pool.py` -- Complementary gate for `util/soak_ledger.py`'s `analyse()` date-pool. `tests/test_soak_ledger.py` hard-codes one `ts` and never asks whether a §15.4 split would change the verdict; ml#1728 pins `pick_next`'s post-intervention filter; ml#1725 pins that the spend-control token is `analyse()["verdict"]`. This suite drives synthetic pre/post rows and pins that pre 24/35 is INCONCLUSIVE, post-only 2/8 is IN-PROGRESS, pooled 26/43 is BET-FAILING (neither side is), `status` first-token is the pooled verdict, and empty/missing `ts` still count — the opposite of the picker. Hermetic — never launches `claude`, never reads the live ledger.
