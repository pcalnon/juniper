...
- `tests/test_soak_ledger_status_token.py` -- Complementary gate for the soak spend-control handshake.
  `util/soak_run_probe.py` takes the verdict as `(status.stdout.split() or [""])[0]`; #1690 and #1703 stub that token, and `tests/test_soak_ledger.py` only asserts `NO-DATA` / `DEGRADED` appear *somewhere* in stdout.
  This suite drives the real `status` CLI against synthetic ledgers and pins token 0 == `analyse()["verdict"]` for every verdict family (BET-FAILING / HOLDS-AT-* / INCONCLUSIVE / IN-PROGRESS / NO-DATA / DEGRADED / NO-SEEDED-DATA), that an open escalation does not steal token 0, and that default status is not JSON.
  Hermetic -- never launches `claude`, never reads the live corpus.
