...
- `tests/test_worktree_inuse_probe.py` -- Hermetic gate for `util/ad-hoc/2026-09-02_worktree_inuse_probe.py` (#1579): STRONG cwd/open-fd REFUSE vs WEAK cmdline CAUTION (exit code stays 0), self/parent argv excluded from weak, sibling prefix near-miss (`foo-extra` is not inside `foo`), unreadable `/proc` counted not treated as in-use. Walks a fake `proc_root`; never the host `/proc`.
...
- `util/ad-hoc/2026-09-02_worktree_inuse_probe.py` -- Independent second opinion for the worktree sweep (#1579). STRONG hits (cwd or an open fd inside the tree) exit 1 REFUSE; WEAK hits (cmdline substring) print CAUTION and do not set the exit code; this process and its parent are excluded from weak by pid so the probe's own argv cannot report every tree in use. Optional `proc_root` (default `/proc`) is a test seam only.
...
│   ├── test_worktree_inuse_probe.py      # Independent in-use probe: STRONG cwd/fd vs WEAK cmdline (hermetic proc_root)
