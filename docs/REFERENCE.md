...
| `FATAL: … tmpfs (RAM-backed)` | Point `DUPLICATI_TEMP_DIR` at a disk-backed sibling of the destination, not `/tmp` |
...
|---------|------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 0.6.9   | 2026-08-24 | Scheduled Duplicati backup lane (#1292): `systemd --user` timer, copy-not-symlink installer, fail-closed dest/tmpfs/passphrase guards, skip-escalation, `--no-auto-compact` |
