...
- `tests/test_register_status_crosscheck.py` -- Hermetic gate for `util/ad-hoc/register_status_crosscheck.py`, the independent third reading of the defect register. `register_open_set.py` and `grep -cE '\\*\\*FIXED'` both read the SAME §4 rows, so they can agree and still both be wrong. Pins three-set AGREE, each missed-touch DISAGREE (table-fixed missing from §2 / §5.1; §2 or §5.1 listing an id whose §4 status cell is still open), dagger / `001a` ids, the status-cell `**FIXED` rule (a §5.1 verification mentioning `**FIXED` must not close an open row -- the whole-line search was the false-AGREE), the count-agnostic `have since been fixed**` status line, and missing-heading exit 1. `util/` is not pre-commit-lint-gated; this suite is the gate.
