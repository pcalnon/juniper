...
- `tests/test_duplicati_scheduled_backup.py` -- Hermetic guards / skip-vs-stale-escalate / installer / OnFailure reporter for the scheduled-backup lane (`util/duplicati_scheduled_backup.bash`, `util/duplicati_backup_failure.bash`, `util/install_duplicati_timer.bash`, `util/systemd/duplicati-backup*`). PATH-stubbed `mountpoint`/`duplicati-cli`/`journalctl`/`notify-send`/`loginctl`/`systemctl`; never touches a real Duplicati process, mount, or user systemd. Sole gate (`util/` is not pre-commit-lint-gated).
...
│   ├── test_duplicati_scheduled_backup.py # Scheduled-backup lane: guards, skip/escalate, installer, OnFailure reporter (hermetic PATH stubs)
