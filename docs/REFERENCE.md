...
- `tests/test_e2e_f037_render_census.py` -- Hermetic gate for the F-CANOPY-037 multi-session render census `util/ad-hoc/e2e_f037_render_census.py` (`util/` is outside every pre-commit Python hook).
  Pins the vacuity contract (`hidden_units` 0/absent → `scope=invalid`, cannot read PASS or FAIL) and the documented conflation (idle populated is VALID and must not be thrown out; growth requires distinct observed topologies).
  Also pins the walk-up Juniper root that must contain BOTH `juniper-canopy` and `juniper-cascor` (three hops from a nested worktree recorded `sha=None`), structured `topodiag` JSON as the only verdict source (stdout that says PASS cannot clean a FAIL / missing file), and the exit contract (2 when a session produced no verdict; 0 when every session is PASS or FAIL, even if painted==0 — the census does not judge the render rate).
  `subprocess` / `run_session` mocked; no live canopy.
