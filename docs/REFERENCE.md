...
- `tests/test_equities_symbol_cap_operator.py` -- Operator-surface gate for the shipped 14-symbol equities refuse (data#354). The refuse lives in juniper-data; this pins the juniper-ml surfaces that can still silently defeat it: a suite cell with `equities` / `equities_seq` must carry an explicit `symbols` list of length 1–14 or `allow_truncation: true` (`max_symbols` alone still 422s against the bundled 503 names), a cascor cell must not stage `equities_seq` (not in `STAGEABLE_GENERATOR_ALIASES`), and `experiment_stack.bash` `data_up` sets `JUNIPER_DATA_EQUITIES_CACHE_DIR` but must not assign `JUNIPER_DATA_EQUITIES_MAX_SYMBOLS` / `_ALLOW_TRUNCATION`. Complementary to `test_experiment_suite_yamls.py` (never looks at generator/symbols) and `test_experiment_stack_script.py` (only asserts the cache dir is present). Hermetic YAML + script-text; no live data service.
...
│   ├── test_equities_symbol_cap_operator.py # Operator surface for the 14-symbol refuse (data#354): suite cells stay inside the cap or opt in; cascor must not stage equities_seq; experiment_stack sets the cache dir and inherits the ceiling / opt-in
