...
- `tests/test_soak_probe_evidence.py` -- Hermetic gate for `util/ad-hoc/2026-08-21_soak_probe_evidence.py` (the independent retrieval-evidence extractor the pointer-follow soak scorer is supposed to see). Pins that reciting `docs/REFERENCE.md` in assistant prose is **not** a retrieval (the #1644 / P15 class), that `tool_result` content still counts (the 2026-08-21 `grep -rn docs/` false-negative), that touching `conf/soak_probes.json` or the protocol-doc filename contaminates, and that CLI stdout never echoes transcript message text. `util/` is outside every pre-commit Python hook, so this suite is the gate.
