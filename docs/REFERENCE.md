...
- `tests/test_register_close_protocol.py` -- Complementary leftover `tests/test_register_open_set.py` (#1648) and `tests/test_register_status_crosscheck.py` (#1717) cannot see. Those suites pin each script in isolation; #1717 then *explicitly* lets `main()` exit 0 or 1 on the living register. This suite pins the operator pair the close protocol actually runs: the committed register must AGREE, and the open-set headline fixed-count must match the crosscheck's §4 `**FIXED` count. Also pins that the first matching §2 status line wins (a later `have since been fixed**` decoy must not steal ids) and that a §5.1 first cell `APD-CASCOR-003 (partial)` still extracts the id. No production edits.
...
│   ├── test_register_close_protocol.py     # Complementary leftover #1648/#1717 cannot see: live AGREE + open-set/crosscheck count lockstep, first-status-line-wins, §5.1 (partial) first-cell; hermetic + live pin
│   └── fixtures/
