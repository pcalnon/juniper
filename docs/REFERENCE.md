...
- `tests/test_e2e_finding_triage_priority.py` -- Complementary leftover of `tests/test_e2e_finding_triage.py` (#1642).
  That suite pins dispositions and puts the priority as the first (or only) severity token in the parenthetical, so it cannot see the F-CANOPY-037 close: `pri_of` takes the first `\b(P0/P1|P0|P1|…)\b` anywhere in the header prose, and a sentence that says "holding the arc's only P0/P1 open" before `(LEDGER; …)` is triaged **P0/P1**.
  Also pins the "not a P2" footgun, word-boundary / case, and that `main()` calls the hoisted `pri_of`. Hermetic temp ledgers; the real evidence note is never read.
- `tests/test_e2e_topology_step_order.py` -- Complementary leftover of `tests/test_e2e_topology_score_contracts.py` (#1676). That suite pins M-TOPOLOGY-18 INDETERMINATE scoring, not the `--step` loop that walks the operator's `wanted` list. Pins that `topostate,topo` stays that order (not STEPS insertion order), unknown names are rejected, empty tokens are dropped, and `main()` iterates `wanted` not `STEPS`. Imports `util/ad-hoc/e2e_topology_step_cli.py` only — never the Playwright driver.
