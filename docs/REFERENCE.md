...
- `tests/test_run_suite_uncountable_report.py` -- Complementary leftover the reader / baseline / compare / kind-edges / gate-metrics suites cannot see: `run_suite.aggregate()` REPORT.md never prints `work invariant: HOLDS` for a recurrence or mixed suite (including planted matching `step_count`s). Third state is `not countable`, distinct from cascor HOLDS / BROKEN / not-measured.
...
│   ├── test_run_suite_uncountable_report.py # Complementary leftover the reader/baseline/compare/kind-edges/gate-metrics suites cannot see: REPORT.md work-invariant third state (not countable ≠ HOLDS), planted matching counts, mixed cascor+recurrence (hermetic)
