...
- `tests/test_stats_summary_render.py` -- Hermetic leftover for `util/experiments/stats_summary.py`'s `summary.md` renderer. `StatsSummaryUnitTest` pins math; `MetricsScrapedTest` pins collection. This file pins the #1550 honesty line operators read: `target_file_written` must not stand in for `scrape_confirmed`; a present `None` is "could not ask", not `False`; a missing key is `n/a (pre-2026-09-01 manifest)`. Also pins last_count==0 overall_mean, non-advancing count skip, git dirty/unavailable, cascor empty per_round, crossval vs disabled, explicit theta.
...
│   ├── test_stats_summary_render.py        # Leftover: util/experiments/stats_summary.py summary.md honesty (#1550 scrape tri-state + last_count==0 + git dirty/unavailable + cascor empty per_round + crossval/theta)
