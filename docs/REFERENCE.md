...
- `tests/test_e2e_matrix_rescore.py` -- Hermetic gate for `util/ad-hoc/e2e_matrix_rescore.py`, the named-row re-score writer Phase 2 uses instead of `e2e_matrix_fill.py --overwrite`. Pins that `pending` is refused (exit 2), dry-run writes nothing, `--write` updates only named `--row` ids (prefix near-miss `M-TOPOLOGY-01` vs `M-TOPOLOGY-010` stays put), a status containing `|` is refused so cell-count cannot shift Status into FA (the C2.2-04 class), escaped `\\|` survives in another cell, Status is located by header name (a short table must not overwrite Notes), and an already-filled status IS overwritten -- that is the re-score contract.
...
│   ├── test_e2e_matrix_rescore.py          # Behavioural: util/ad-hoc/e2e_matrix_rescore.py named-row re-score (pending refuse, dry-run, prefix near-miss, cell-count refuse, escaped pipe, status-by-header, overwrite contract)
│   └── fixtures/
