...
- `tests/test_compare_baseline_defects.py` -- Complementary hermetic tests for `util/experiments/compare_baseline.py` defects A1–A7 (handoff `HANDOFF_2026-09-04_perf-lane-gate-built-waves-not-closed.md`). The happy-path matrix in `tests/test_compare_baseline.py` only constructs `succeeded` cells with histograms; these pin the leftover those tests cannot see: 4-of-5 unmeasured still looking invariant, `timed_out` cells, an extra unreadable suite converting FAIL(1) into REFUSED(2), zero-work `bool([0.0])`, partial scenario coverage, and duplicate-fingerprint collapse. `util/` is not pre-commit-lint-gated, so this unittest is the gate.
...
│   ├── test_compare_baseline_defects.py  # Complementary: compare_baseline A1–A7 refusals the happy-path matrix cannot see (unmeasured / timed_out / FAIL-vs-REFUSED / zero-work / partial coverage / duplicate fingerprints; hermetic)
