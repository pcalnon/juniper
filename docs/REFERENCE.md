...
- `tests/test_soak_run_probe_stopping_rule.py` -- Complementary gate for `util/soak_run_probe.py`'s terminal-verdict spend control. `tests/test_soak_run_probe.py` already pins the helper and a BET-FAILING `--dry-run` walk through `main()`; this suite pins what those members cannot see: a real run refuses *before* `dispatch` (a `dry_run=True` smuggle keeps the sibling suite green), live `status` exit codes are ignored (1 for BET-FAILING *and* INCONCLUSIVE+escalations; 2 for DEGRADED / NO-DATA / NO-SEEDED-DATA, which fail open), `verdict_is_terminal` is prefix-only and case-sensitive, and only `stdout.split()[0]` is the verdict. Hermetic -- `dispatch` is stubbed; never launches `claude`.
