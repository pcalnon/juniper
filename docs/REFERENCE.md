...
- `tests/test_e2e_row_coverage.py` -- Hermetic tests for `util/ad-hoc/e2e_row_coverage.py`, the compressed-range ESTIMATOR for remaining canopy E2E matrix rows. Distinct from `e2e_unfilled_rows.py` (the AUTHORITATIVE status-cell reader; Segment 15 planned two already-PASS rows from this estimator's over-credit of TSV `pending` records). Pins inclusive `01..06` expansion (and that `01..06,09` does not invent 07/08), first-field-only so prose mentions cannot shrink remaining, unmatched tokens staying in `unknown` rather than `hit`, lane-suffix folding that does not eat `-01`, and characterization that TSV `pending` records ARE credited. `util/` is outside every pre-commit Python hook, so this suite is the gate.
