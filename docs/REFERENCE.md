...
- `tests/test_canopy_poller_inventory.py` -- Hermetic AST-census pins for `util/ad-hoc/canopy_poller_inventory.py`, the F-CANOPY-027 interval-callback inventory that sizes dash-renderer's 12-slot starvation budget. `util/` is not pre-commit-lint-gated, so this suite is the gate. Synthetic `.py` fixtures only (no canopy checkout).
  - Pins the designed resolve path (`f"{self.component_id}-…"` via the `__init__` default), that a click Input is not a poller, that `visualization-tabs` gating is exact (Input vs State vs a near-miss id), and that two intervals on one callback are both counted.
  - Characterises the known undercount (unresolvable f-string / body-assigned id / bare `@callback` Name / keyword `Input(...)` are invisible -- the 151-of-182 hole the sibling budget probe already records). A miss under-states concurrent load, the same class as the X7 gate that certified a partial fix.
