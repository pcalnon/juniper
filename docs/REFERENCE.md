...
- `tests/test_e2e_topology_row_predicates.py` -- Hermetic gate for `util/ad-hoc/e2e_topology_row_predicates.py`, the three M-TOPOLOGY row predicates that scored the easier half of an OR. Pins that M-TOPOLOGY-06 requires label AND hidden-count (the old `OR` PASSed on the stats-bar branch while the slider label sat at `"0 of 40"` — F-CANOPY-042, found by eye); that M-TOPOLOGY-07 asserts the label, not display alone; that M-TOPOLOGY-12 is BLOCKED (not FAIL) when the Clear-selection control is absent (canopy#573 restated the withdrawn empty-space gesture). The Playwright driver is checked structurally for the two call sites; `util/` is outside every pre-commit Python hook, so this suite is the gate.
