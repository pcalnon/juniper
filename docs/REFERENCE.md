...
- `tests/test_recurrence_kind_edges.py` -- Complementary leftover RecurrenceKindTest, RecurrenceRefusalTest, and `tests/test_work_countable_contract.py` cannot see.
  Pins that recurrence `train` is never aliased onto `drive_seconds`; kind detection is key presence (`drive: null` stays cascor; `train: null` is still recurrence);
  `render()` never prints WORK INVARIANT HOLDS for an uncountable suite; CLI `--run` JSON emits kind/work_countable; a non-mapping `timings`/`dataset` does not crash `read_run`;
  and `run_suite._gate_metrics` carries `work_countable=False` without inventing `step_count` from `n_epochs`.
...
│   ├── test_recurrence_kind_edges.py     # Complementary leftover RecurrenceKindTest / RecurrenceRefusalTest / test_work_countable_contract.py cannot see: train≠drive alias, drive:null key-presence, train:null still recurrence, render never HOLDS, CLI --run JSON, non-mapping timings/dataset, _gate_metrics work_countable=False (hermetic)
