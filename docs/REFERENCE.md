...
│   ├── test_run_suite_gate_metrics.py    # Complementary leftover ml#1643 cannot see: `_gate_metrics` VALUES (not hardcoded headers), HOLDS vs BROKEN vs not-measured, single-workload yes vs NO, and that the sibling import is not wrapped in ImportError (blank columns look like an unmeasured suite)
