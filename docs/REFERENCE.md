...
- `tests/test_matrix_set_verdicts.py` -- Hermetic gate for `util/ad-hoc/2026-09-02_matrix_set_verdicts.py`, the only guarded write into the 298-row canopy E2E matrix (`util/` is outside every pre-commit Python hook). Tempfile fixtures only; never opens the live matrix.
  - Pins `--from` mismatch and missing-id refuse with the file bytes unchanged, and atomic fail-closed (one bad `--set` among two writes neither row).
  - Exact row-id match so `M-TOPOLOGY-09` cannot retarget `M-TOPOLOGY-090` / `-10`; an id that appears only in a later cell is not the row identity.
  - Bad `--set` syntax exits 2. Status is the LAST data cell: an earlier `AUTO` column is not overwritten.
