...
- `tests/test_termination_branch_precondition.py` -- Complementary leftover of the termination-branch guard (`step_count` is exact only WITHIN a `completion_reason`). The comparator suite pins the five-case counterexample; this suite pins the reader/summarise fields, `make_baseline`'s truncated/mixed refusals, the baseline-side fail-closed, a same-count branch flip, and the partially-annotated suite that would otherwise collapse to a unanimous branch.
...
│   ├── test_termination_branch_precondition.py  # Complementary leftover of the termination-branch guard: reader/summarise fields, make_baseline truncated/mixed refusals, baseline-side fail-closed, same-count branch flip, partially-annotated suite
