...
- `tests/test_e2e_append_statuses.py` -- Hermetic gate for the canopy E2E TSV verdict appender `util/ad-hoc/e2e_append_statuses.py` (`util/` is outside every pre-commit Python hook).
  Default skip-existing refuses a double-count; `--replace` rewrites the named row in place (file order kept); prefix near-miss (`M-TOPOLOGY-01` vs `-010`) is exact-id on both skip and replace; tabs/newlines inside a field collapse so one input row stays one TSV line; missing TSV / non-array JSON refuse with rc 2 and write nothing.
  Distinct from `e2e_matrix_fill.py` (bulk matrix writer), `e2e_matrix_rescore.py` (named-row re-score), and `2026-09-02_matrix_set_verdicts.py` (single-row `--from`/`--set`). Never opens a live run directory.
...
- `util/ad-hoc/e2e_append_statuses.py` -- Dup-guarded append of verdict rows into a Phase-1 run's `statuses.tsv`. Default skips an already-present `row_id` so a re-run cannot double-count a lane; `--replace` rewrites that row in place. Tabs/newlines inside any field collapse to spaces so one row stays one TSV line. Tests: `tests/test_e2e_append_statuses.py`.
