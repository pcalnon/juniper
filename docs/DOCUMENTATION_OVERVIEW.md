...
| **Prune stale local `fix/*` (no `.claude/` tree)** | [REFERENCE — Stale fix/* Branch Prune](REFERENCE.md#stale-fix-branch-prune)                                                                              | docs/    |
...
| **REFERENCE.md**                       | Reference  | Extras, compatibility, host-stack ops, agent-suite doctor, stale `fix/*` prune, sibling packages, and release-workflow reference |
