...
- `util/memory_budget_check.py` -- Character-ceiling gate on always-loaded memory files (`conf/memory_budget.json`; ADVISORY `Memory Budget` job). **Don't grow `AGENTS.md` — relocate to `docs/REFERENCE.md` and leave a pointer that keeps an accurate open/closed status.** `Allow-Budget-Overrun:` is a loan, not a pass. [Detail](docs/REFERENCE.md#memory-file-size-budget); tests: `tests/test_memory_budget_check.py`.
