...
- `util/reap_pytest_orphans.bash` -- Safely reaps orphaned Juniper pytest multiprocessing children. Supports `JUNIPER_REAP_PROC_ROOT` and `JUNIPER_REAP_KILL_CMD` test hooks for deterministic regression tests.
  - Candidate awk gate: current user + `python` + (`JuniperC[a-z0-9]+` or `Juniper/worktrees/`) only — never other users or plain non-Juniper pytest. Empty set → `No Juniper python processes found.` / exit 0.
  - `SKIPPED` (not WOULD REAP): `/proc/<pid>` gone after `ps`, or status missing `PPid:`. Orphan = ppid 1 / user `systemd --user` / missing parent; live parent → KEEP. Operator guide: `docs/REFERENCE.md` § Pytest Orphan Reaper.
...
- `tests/test_reap_pytest_orphans.py` -- Tests for `util/reap_pytest_orphans.bash` dry-run, live-parent safety, orphan detection, and isolated kill invocation
  - Candidate awk exclusions (other-user / non-Juniper python) + empty-candidate exit 0 + SKIPPED arms (ps→gone, missing `PPid:`) pinned in open juniper-ml#784.
