...
- `util/safe_merge.py` -- Merge gate (synthesis R4): merges a PR **only** after its REQUIRED checks have finished green, refusing otherwise. Closes the measured failure where **12% of freshness-synced PRs merged before the re-test they had just paid for could finish** -- ml#932 merged 66 s after its sync on a head with zero CI check-runs, ml#924 merged 25 s after its update-branch head; both then reddened `main`. That is strictly worse than not syncing, because a stale-but-tested head is replaced by a fresh-but-untested one.
  ...
  - The guard earned its place on the FIRST live run (ml#1170), which was rejected with *"Head branch was modified"*. Cause: `update-branch` answers **202 Accepted** and moves the ref **asynchronously**, so reading the PR immediately returns the OLD head -- the tool then waited on that head's already-green checks and tried to merge a SHA that no longer existed. `update_branch` now polls until the ref actually moves, and a rejection of that kind is mapped to a **refusal** (exit 1), not a hard error: nothing was merged, which is correct.
  ...
  - **Not enforcement.** A script can be skipped; the owner's `always` ruleset bypass is what makes required checks advisory for that actor. `python util/safe_merge.py --pr N [--repo R] [--execute]`; **`--dry-run` is the default**. Exit 0 merged / 1 refused / 2 misuse / 3 hard error. Tests: `tests/test_safe_merge.py`.
...
- `tests/test_safe_merge.py` -- Tests for `util/safe_merge.py` (the R4 merge gate). Hermetic: `_gh` and `wait_for_required` are replaced with recorders, so no network / `gh` / repo / PR is touched. Pins the safety contract -- every refusal path (closed / draft / conflicted / checks-failed / checks-unfinished / sync-cycles-exhausted / merge-returned-but-PR-not-merged) asserts that **no** `pr merge` was issued, plus the load-bearing `--match-head-commit` pinning, the server-side (`PUT`) branch refresh, the no-local-git invariant, and dry-run writing nothing.
