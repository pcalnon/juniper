...
- `util/experiments/run_experiment.py` -- Single-run experiment driver (plan §6.3; Wave 2.2 = the cascor **service** path, Wave 2.3 = the recurrence **service** path, Waves 2.4/2.5 = the §8.1/§8.2 plot sets via `plots_cascor.py` / `plots_recurrence.py` (2.5 closes G-5), Wave 2.6 = the §8.3 stats/summary via `stats_summary.py`).
  ...
  - Plot semantics: applicability skips + renderer `ValueError` (no-renderable-data) = recorded per-plot SKIP only (exit 0, no PNG); a non-ValueError render error / failed fetch / missing matplotlib on a requested plot = SKIP **and** acceptance failure (exit 1).
  - Soft plot edges: misaligned optional `target_dt` omits the residual-vs-dt panel (not a raise); empty `eval_aggregate` falls back to `folds[0].eval_metrics`. Manifest `driver.plots` records requested/rendered/skipped. Operator table: [`docs/REFERENCE.md` § Plot SKIP vs acceptance](docs/REFERENCE.md#plot-skip-vs-acceptance-valueerror-contract).
