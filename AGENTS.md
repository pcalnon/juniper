...
- `util/reap_pytest_orphans.bash` -- Safely reaps orphaned Juniper pytest multiprocessing children (`--dry-run` / `--verbose`).
  ...
  - Test hooks: `JUNIPER_REAP_PROC_ROOT`, `JUNIPER_REAP_KILL_CMD`. Operator surface: [docs/REFERENCE.md § Pytest Orphan Reaper](docs/REFERENCE.md#pytest-orphan-reaper).
...
- `util/fleet_triage/predict_merge.py` -- Deterministic predicted-merge triage for third-party fleet PRs (Stage-0 supervisor script layer; flood §4 item 7). Per PR, in a throwaway detached `git clone` under the system tempdir (never a `git worktree`, never a push), merges `origin/main` into the branch tip and on the result runs the repo-pinned fast gates + an AST symbol-loss screen + a docs additions-only screen. `--pr N | --batch [--json] [--repo-root P]`; exit 0/2. Tests: `tests/test_predict_merge.py`.
  - Emits per-PR JSON (`verdict` MERGE-CLEAN / NEEDS-UPDATE-BRANCH / DAMAGED-FIX-FIRST / CONFLICT / batch-only `ERROR` + the TRUE changed-file delta from the merge result, NOT `gh --json files`); `--batch` builds the same-file cluster map + a restore/heal-first, least-colliding merge order.
  - `--pr` / `triage_pr`: `gh` nonzero or non-JSON raises `PredictMergeError` → CLI exit `2` (hard-fail). `--batch` / `triage_batch`: soft-`ERROR` that row and continue (juniper-ml#977 / #930).
  - Gate battery uses `changed_existing` (`_blob(HEAD)` present) — deleted `.py` stays in `true_delta` for the symbol screen but is never passed to `pre-commit --files`. `JUNIPER_FLEET_SKIP_PRECOMMIT=1` forces hook `skip_all`.
  ...
  - Control WS: `ws_control_rate_limit_per_sec=0` → `LeakyBucket(refill_rate=0)`; open [#977](https://github.com/pcalnon/juniper-ml/pull/977) makes `retry_after` return `3600` (pre-fix ZeroDivisionError). See [`docs/REFERENCE.md` § Control WS LeakyBucket](docs/REFERENCE.md#control-ws-leakybucket-ws_control_rate_limit_per_sec).
...
- `tests/test_run_experiment.py` -- Hermetic tests for `util/experiments/run_experiment.py` (CLI experimentation plan Waves 2.2-2.6: the cascor + recurrence service paths, the §8.1 + §8.2 plot sets, and the §8.3 stats/summary renderers (e2e stats assertions for both kinds + every-outcome coverage + the `StatsSummaryUnitTest` percentile/delta/grouping/degraded-notes units) --
  ...
  shape-assert pass/mismatch, W-3 refusal for gaussian), the recurrence path (synchronous train 200/409/422/socket-timeout arms, predict/crossval `dataset_id` refs +
