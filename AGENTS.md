...
- `util/juniper_plant_all.bash` -- Starts all Juniper ecosystem services. `JUNIPER_CASCOR_HOST` defaults to `localhost` and `JUNIPER_CASCOR_PORT` defaults to `8201`; both can be overridden via the environment (e.g. `JUNIPER_CASCOR_HOST=remote.example.com JUNIPER_CASCOR_PORT=8201 util/juniper_plant_all.bash`).
  - Health interval clamp (juniper-ml#782): `wait_for_health` rejects non-positive / non-integer `HEALTH_CHECK_INTERVAL` (and a zero/invalid 4th arg) with a WARNING and clamps to `1s` — `0` previously busy-looped forever (`sleep 0` never advances elapsed). Prefer default `2`. Operator details: [`docs/REFERENCE.md` Host Orchestration](docs/REFERENCE.md#host-orchestration-utilities). Tests: `tests/test_juniper_plant_all.py` (`TestWaitForHealth`).
