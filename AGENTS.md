...
- `util/juniper_chop_all.bash` -- Stops all Juniper ecosystem services from `JuniperProject.pid` (`SIGTERM_TIMEOUT` default 15; `KILL_WORKERS`; `--systemd` / `USE_SYSTEMD`).
  - JR-ML-SEC-045 `validate_pid` (juniper-ml#777): case-folded hyphen/underscore-stripped token match on `${JUNIPER_CHOP_PROC_ROOT:-/proc}/<pid>/cmdline` accepts plant cascor/canopy (`JuniperCascor1`/`JuniperCanopy1` + `server.py`/`main.py`); rejects reused/wrong PIDs and `juniper-cascor`↔worker prefix collisions. Reject skips (no `STOP_FAILURES` bump) — false reject can clear the pidfile while leaving the process up.
  - `graceful_stop` (juniper-ml#778 coverage): SIGTERM→wait→SIGKILL; already-exited returns 0; `STOP_FAILURES>0` preserves the pidfile. Operator surface: `docs/REFERENCE.md` Host Orchestration + cheatsheet tip. Tests: `tests/test_juniper_chop_all.py`.
