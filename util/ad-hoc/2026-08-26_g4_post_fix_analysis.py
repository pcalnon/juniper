...
def count_in_logs(log_dir: Path, pattern: re.Pattern[str]) -> tuple[int, list[str]]:
...
def main(argv: list[str]) -> int:
    ...
    cli_dirs = sorted(p for p in out_root.glob("cli-*") if p.is_dir())
    ...
    cli_total = 0
    for i, d in enumerate(cli_dirs, start=1):
        n, files = count_in_logs(d / "logs", pattern)
        cli_total += n
        print(f"  cli     pair {i}: {n:3d}   files={files}")
    svc_total = 0
    for pair, run_dir, _ in service_runs:
        n, files = count_in_logs(run_dir / "logs", pattern)
        svc_total += n
        print(f"  service pair {pair}: {n:3d}   files={files}")
