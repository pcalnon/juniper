...
from the docs census ``util/ad-hoc/2026-07-28_docs_census_v2_c2.py`` (Proposal P2 S2).
The flood's docs class (#6 / PR #801 / #803) was a *net section deletion* that no
existing check could see: the doc-links validator only catches dangling anchors, and
markdownlint excludes ``notes/`` + ``docs/``. So a merge that dropped a whole runbook
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            "Default scope (auto-discovery): AGENTS.md (+ CLAUDE.md symlink), docs/**/*.md, notes/**/*.md. "
            "An explicit --files list bypasses the scope filter (any .md path). Exit 0=clean, 1=findings, "
            "2=usage. Escape hatch: a `Allow-Docs-Rewrite: <path>[, ...]` commit trailer in the BASE..HEAD "
            "range waives the enumerated files (`*` waives all)."
        ),
    )
