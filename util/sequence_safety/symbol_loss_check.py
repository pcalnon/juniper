...
Scope (default, auto-discovery). In-scope = top-level ``tests/*.py`` + ``util/**/*.py``
+ ``util/**/*.bash`` -- the census ``in_scope``, the locus of every silent-deletion
incident in the flood. It EXCLUDES ``scripts/``, the sub-package ``juniper-*/`` src
trees, nested ``tests/*/`` subdir ``.py``, and any non-``.py``/``.bash`` file. An
explicit ``--files`` list bypasses the scope filter (any ``.py`` / ``.bash`` path).
...
def changed_files(root: str, base: str, head: str) -> list[str]:
    ...
    recipe the archive-guard uses (``ci.yml:452``). For the intended invocations
    (a PR merge ref; G3 ``<merge>^1``..``<merge>``) base is an ancestor of head, so
    the three-dot range degenerates to the plain ``base..head`` change set.
    """
...
def apply_relocation(findings: list[Finding], base_invs: dict[str, dict[str, Sym]], head_invs: dict[str, dict[str, Sym]]) -> None:
    ...
    head_new_bodies: set[str] = {
        s.body for inv in head_invs.values() for s in inv.values() if s.body and s.body not in base_bodies and (s.body.count(" ") + 1) >= BODY_SIMILARITY_MIN_LINES
    }
...
def run(root: str, base: str, head: str, files: Optional[list[str]]) -> tuple[int, dict]:
    ...
    report = {
        ...
        "findings": [
            {"path": f.path, "symbol": f.symbol, "verdict": f.verdict, "severity": f.severity, "detail": f.detail}
            for f in sorted(findings, key=lambda x: (x.path, x.symbol))
        ],
    }
...
def main(argv: Optional[list[str]] = None) -> int:
    ap = argparse.ArgumentParser(
        ...
        epilog=(
            "Default scope (auto-discovery): top-level tests/*.py + util/**/*.py + util/**/*.bash. "
            "EXCLUDES scripts/, the sub-package juniper-*/ src trees, nested tests/*/ subdir .py, and "
            "non-.py/.bash files. An explicit --files list bypasses the scope filter. Exit 0=clean, "
            "1=findings, 2=usage. Escape hatch: a `Allow-Symbol-Loss: <qualified.symbol>[, ...]` commit "
            "trailer in the BASE..HEAD range waives the enumerated symbols (a `*` wildcard is rejected)."
        ),
    )
