...
echo "[${JUNIPER_SCRIPT_NAME}:${LINENO}] JUNIPER_CANOPY_SERVER__PORT=\"${JUNIPER_CANOPY_PORT}\" JUNIPER_CANOPY_CASCOR_SERVICE_URL=\"${JUNIPER_CASCOR_URL}\" JUNIPER_CANOPY_JUNIPER_DATA_URL=\"${JUNIPER_DATA_URL_FOR_CANOPY}\" JUNIPER_CANOPY_CASCOR_WS_ORIGIN=\"${JUNIPER_CANOPY_CASCOR_WS_ORIGIN}\" nohup \"${JUNIPER_CANOPY_PYTHON}\" \"${JUNIPER_CANOPY_MODULE}\" >\"${JUNIPER_CANOPY_LOG}\" 2>&1 &"
# F-E2E-004 (canopy E2E ledger): export the port into canopy's process so the
# documented JUNIPER_CANOPY_PORT override actually rebinds the server. Canopy
# reads settings under env_prefix JUNIPER_CANOPY_ with "__" nesting, so
# ServerSettings.port is JUNIPER_CANOPY_SERVER__PORT; the flat variable alone
# only moved this script's health probe and Origin derivation, so an override
# probed a port canopy never bound (the cascor block above exports HOST and
