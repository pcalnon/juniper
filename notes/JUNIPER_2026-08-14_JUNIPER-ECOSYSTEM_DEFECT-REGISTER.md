...
**Eighteen have since been fixed** — `APD-DATA-002`, `APD-DATA-006`, `APD-DATA-034`, `APD-DATA-036`, `APD-CASCOR-002`, (2026-08-16) `APD-DATA-001` † / `APD-CASCOR-004` †, (2026-08-17) `APD-DATA-003` / `APD-CASCOR-006` † plus `APD-SVCCORE-003` / `APD-SVCCORE-010` / `APD-OBS-001`, and (2026-08-20) `APD-DATA-035` † / `APD-CASCOR-001b` plus `APD-CASCOR-001a`, then `APD-DATA-004`, and (2026-08-21) `APD-DCLIENT-001` / `APD-DCLIENT-003` — leaving **78 open**; each is marked at its detail entry and in its §4 table row, and all eighteen are recorded in [§5](#5-fixed-findings-before-and-since-the-primer) with their PR and verification.
...
**The sibling-package-drift group now has its convention** (2026-08-21). `APD-DCLIENT-001` — exceptions carrying no `status_code` / `detail` / `response` — is the same defect as `APD-CCLIENT-004` and `APD-RCLIENT-001` in the two other clients. It was fixed first in `juniper-data-client` ([#158](https://github.com/pcalnon/juniper-data-client/pull/158)) precisely so there is something concrete to port: keyword-only `status_code` / `detail` / `response` on the base class, `detail` holding the server's payload unmodified, a `__reduce__` override so the context survives pickle/copy, and the package's own test double updated to populate the same fields. No drift gate can enforce this — three separately released packages share no code — so the enforcement is that the two remaining entries cite this implementation.
