...
| ~~**D4**~~ | ~~**The armed net drops `--match-head-commit`.**~~ **CLOSED 2026-08-21 by measurement.** The net now pins at arming. Probe ml#1225: armed *with* a pin, pushed a commit to move the head, re-read the PR — `autoMergeRequest` still present, `enabledAt` unchanged. So `expectedHeadOid` on `enablePullRequestAutoMerge` is an **enable-time** guard, not continuous. This had to be measured: if it were continuous, pinning would kill the net the moment GitHub moved the head to satisfy `strict` — exactly when the net matters — silently negating D1. The residual trade **remains and is stated**: after arming, the net merges whatever head is current when checks pass. |
