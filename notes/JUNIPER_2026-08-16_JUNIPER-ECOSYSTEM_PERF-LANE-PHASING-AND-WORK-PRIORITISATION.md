...
| **G4**| **Per-run fixed overhead is now the dominant cost** (NEW, exposed by cascor#563)                                       | **OPEN — no instrument** ([cascor#571](https://github.com/pcalnon/juniper-cascor/issues/571)). Before F1 the candidate phase was 98% of a cap-16 span (890 of 908 s); after, 66% (41 of 62 s). Startup, dataset fetch, output passes and teardown now set the wall and have never been examined. The existing phase split separates candidate from output only, so this needs a new instrument first | no |
