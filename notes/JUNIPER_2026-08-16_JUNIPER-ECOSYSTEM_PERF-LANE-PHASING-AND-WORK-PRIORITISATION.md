...
| **G2**| **Residual CLI-vs-service wall gap** after [cascor#533](https://github.com/pcalnon/juniper-cascor/pull/533)            | **OPEN**, unblocked with a condition. G1 shows the *service* arm is a sound single-run instrument but the *CLI* arm is not, so the measurement must be **k-paired on the CLI side**, with k sized against a noise floor this campaign deliberately did not publish (host contention, see its §5.1). Also newly complicated: the two arms do **not** start from the same state on an identical cell (evidence §4.3) | no |
