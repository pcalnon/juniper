...
| **G6**| **Forkserver isolation is not delivered** ([cascor#570](https://github.com/pcalnon/juniper-cascor/issues/570)) — workers carry their *launcher's* module table (NEW G-id 2026-08-24; previously untracked in this register) | **OPEN.** Workers measure 1,871 (CLI) / 1,410 (service) modules — within a handful of their launchers (1,867 / 1,416), far above a clean forkserver table (1,091 preload, 1,333 with the trainer). The leak route is untraced; check first whether the forkserver is started *after* the launcher has already imported its full graph, which would explain the numbers exactly and make the fix structural rather than about the preload list. **Gates G5** — resolve before starting either #568 or #569 | no |
