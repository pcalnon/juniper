...
**License**: MIT License
**Version**: 0.7.1
**Last Updated**: 2026-08-24 (G1/G1a closed, G1b shipped, G2 work-term reconciled, G6 added — see [post-arc determinism evidence](JUNIPER_2026-08-24_JUNIPER-ECOSYSTEM_CLI-EXPERIMENTATION-POST-ARC-DETERMINISM-EVIDENCE.md))
...
| **G4**| **Per-run fixed overhead is now the dominant cost** (NEW, exposed by cascor#563)                                       | **INSTRUMENT SHIPPED + FIRST MEASUREMENT 2026-08-25** ([cascor#571 comment](https://github.com/pcalnon/juniper-cascor/issues/571#issuecomment-5409644938)): `util/ad-hoc/2026-08-25_g4_overhead_decomposition.py` (stream-order state machine; corrected training boundary — the initial output pass runs BEFORE the `fit: Starting…` record). Measured k=4 paired at cap 16 on `c4bbe815`: the classic fixed-overhead suspects are all SMALL on both arms (startup ~2 s, dataset ~0–1 s, boot ~5 s, teardown ~0–2 s) — **the dominant overhead is one defect: [cascor#586](https://github.com/pcalnon/juniper-cascor/issues/586), the CLI's 7/7 ungraceful worker stops burning ~35 s/run serially at cap 16** (~32% of the CLI training span; 0/7 at cap 4, 0/7 service). The 1.817 span ratio does not reproduce on the current build (nrun-anchored 1.32, ≈ all #586; whole-process 1.09 — the service now spends MORE in candidate and output). After #586, G4 reduces to the cross-arm parity conversation (#582/#572/#530/#578) | no |
| **G5**| **F2 CLI import hygiene / F3 forkserver preload**                                                                     | **OPEN** ([cascor#568](https://github.com/pcalnon/juniper-cascor/issues/568) / [#569](https://github.com/pcalnon/juniper-cascor/issues/569); **gated by G6**). F2: `import main` pulls fastapi+pydantic (1,867 modules vs api.app's 1,416); **demoted** by cascor#563 from perf-critical to hygiene; import edge not traced. F3: preload omits `cascade_correlation` (242 modules / 1.822 s, ~12.8 s per pool creation) — **blocked on a fork-safety audit**, and its sizing assumes workers inherit the forkserver table, which G6 measured they do not. Both may be subsumed by the untraced forkserver-isolation leak (evidence §4.4a §6.1) | no |
| **G6**| **Forkserver isolation is not delivered** ([cascor#570](https://github.com/pcalnon/juniper-cascor/issues/570)) — workers carry their *launcher's* module table (NEW G-id 2026-08-24; previously untracked in this register) | **ROUTE TRACED 2026-08-25** ([evidence comment](https://github.com/pcalnon/juniper-cascor/issues/570#issuecomment-5408936691)): the forkserver itself is CLEAN (measured 1,093 modules, torch+numpy only — preload isolation works). The leak is per-CHILD: CPython's spawn-preparation (`spawn._main → prepare → _fixup_main_from_path`) **re-imports the launcher's `__main__` in every forkserver child** before unpickling — so each candidate worker re-executes `main.py`'s module body (`spiral_problem`→matplotlib, and the **module-scope Sentry init at `main.py:188` → fastapi/httpx via integration autoload**) reaching 1,872 pre-task; the service's 1,410 ≈ its uvicorn entry the same way. **New defect: every worker initializes Sentry.** The "started-late" hypothesis is dead — start timing is irrelevant to a spawn-clean forkserver. Fix order (still **gates G5**): (1) `main.py` re-import hygiene — the substance of #568, whose import edge is hereby answered; (2) lazy `CascadeCorrelationPlotter` import; (3) only then preload `cascade_correlation` (#569) — preloading before (2) would poison the forkserver with matplotlib | no |
